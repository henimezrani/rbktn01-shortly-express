...
describe("", function () {
  ...
  describe("Account Creation:", function () {
    it("signup creates a new user record", function (done) {
      var options = {
        method: "POST",
        uri: "http://127.0.0.1:4570/signup",
        json: {
          username: "Samantha",
          password: "Samantha"
        }
  ...
  describe("Sessions and cookies", function () {
    ...
    it("saves a new session when the server receives a request", function (done) {
      requestWithSession("http://127.0.0.1:4570/", function (err, res, body) {
        if (err) {
          return done(err);
        }
        var queryString = "SELECT * FROM sessions";
    ...
    it("sets and stores a cookie on the client", function (done) {
      requestWithSession("http://127.0.0.1:4570/", function (error, res, body) {
        if (error) {
          return done(error);
        }
        var cookies = cookieJar.getCookies("http://127.0.0.1:4570/");
    ...
    it("destroys session and cookie when logs out", function (done) {
      addUser(function (err, res, body) {
        ...
        requestWithSession("http://127.0.0.1:4570/logout", function (error, response, resBody) {
          if (error) {
            return done(error);
          }
  ...
  describe("Privileged Access:", function () {
    it("Redirects to login page if a user tries to access the main page and is not signed in", function (done) {
      request("http://127.0.0.1:4570/", function (error, res, body) {
        if (error) {
          return done(error);
        }
    ...
    it("Redirects to login page if a user tries to access the create page and is not signed in", function (done) {
      request("http://127.0.0.1:4570/create", function (error, res, body) {
        if (error) {
          return done(error);
        }
        expect(res.req.path).to.equal("/login");
    ...
    it("Redirects to login page if a user tries to see all of the links and is not signed in", function (done) {
      request("http://127.0.0.1:4570/links", function (error, res, body) {
        if (error) {
          return done(error);
        }
        expect(res.req.path).to.equal("/login");
  ...
  describe("Link creation:", function () {
    ...
    afterEach(function (done) {
      requestWithSession("http://127.0.0.1:4570/logout", done);
    });
    ...
    describe("With previously saved urls:", function () {
      ...
      it("Shortcode redirects to correct url", function (done) {
        var options = {
          method: "GET",
          uri: "http://127.0.0.1:4570/" + link.code
        };
      ...
      it("Shortcode redirects to index if shortcode does not exist", function (done) {
        var options = {
          method: "GET",
          uri: "http://127.0.0.1:4570/doesNotExist"
        };
        ...
        requestWithSession(options, function (error, res, body) {
          ...
          var currentLocation = res.request.href;
          expect(currentLocation).to.equal("http://127.0.0.1:4570/");
          done();
        });
      });
